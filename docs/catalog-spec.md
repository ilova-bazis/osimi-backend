...
| `object_id` | string \| null | May be `null` during ingestion-stage lease payloads; required for finalized object catalogs |
...
- [ ] `object_id` matches folder name when present (may be `null` during ingestion-stage lease payloads)
