...
function mapIngestionItemFile(
    row: IngestionItemFileRow,
): IngestionItemFileRecord {
    return {
        id: row.id,
...
function mapLeasedIngestionFile(
    row: LeasedIngestionFileRow,
): LeasedIngestionFileRecord {
    return {
        id: row.id,
    ...
    const reorder = async (
      ...
      `;
            ...
            const providedIds = new Set(
                params.items.map((row) => row.ingestionItemId),
            );
    ...
    const reorder = async (
      ...
      `;
            ...
            const existingIds = new Set(
                existingRows.map((row) => row.ingestion_file_id),
            );
            const providedIds = new Set(
                params.files.map((row) => row.ingestionFileId),
            );
